...
!For Level Set
INTEGER  :: LIMITER_LS,LU_CRWN_PROB_LS,LU_FLI_LS,LU_ROSX_LS,LU_ROSY_LS,LU_SLCF_LS,LU_TOA_LS,NX_LS,NY_LS
...
!       LP%VEG_CHAR_MASS  = PC%VEG_BULK_DENSITY*PC%VEG_CHAR_FRACTION
        ...
        ENDIF
        LP%VEG_EMISS = 0.85_EB*4._EB*SIGMA*LP%VEG_KAPPA*LP%TMP**4
        LP%VEG_DIVQR = 0.0_EB
...
REAL(EB) :: CHAR_FCTR,CHAR_FCTR2,CP_VEG,DTMP_VEG,MPV_MOIST,MPV_MOIST_MIN,DMPV_VEG,MPV_VEG,MPV_VEG_MIN, &
            SV_VEG,TMP_VEG,TMP_VEG_NEW,TMP_FILM,CM,CN
REAL(EB) :: TMP_IGNITOR
...
!print*,rho_gas,qrel,sv_veg,mu_air
 ...
 IF(RE_D<4) THEN
    CN=0.989_EB
    CM=0.33_EB
 ELSE IF (RE_D.GE.4 .AND. RE_D.LT.40) THEN
    CN=0.911_EB
    CM=0.385_EB
 ELSE
    CN=0.683_EB
    CM=0.466_EB
 ENDIF
 QCON_VEG = 0.25_EB*SV_VEG*K_AIR*CN*RE_D**CM*PR_ONTH*TMP_GMV !W/m^2 from Porterie (cylinder)
...
!1111 format(2x,5(e15.5))
 ...
 LP%TMP = TMP_VEG_NEW
 LP%VEG_EMISS = 0.85_EB*4.*SIGMA*LP%VEG_KAPPA*LP%TMP**4 !used in RTE solver
...
REAL(EB) :: CP_ASH,CP_CHAR,CP_H2O,CP_VEG,CP_TOTAL,DTMP_VEG,Q_VEG_CHAR,TMP_VEG,TMP_VEG_NEW, &
            CHAR_ENTHALPY_FRACTION_VEG,TMP_FILM,CM,CN
...
!-- Most recent approach, drag constant can vary with height, if hveg > dzgrid
  VEG_DRAG(IIG,JJG,:) = 0.0_EB
  IF (WC%VEG_HEIGHT > 0.0_EB) THEN
...
!print*,rho_gas,qrel,sv_veg,mu_air
  ...
  DO I=1,NVEG_L-LBURN
    ...
    TMP_FILM = (TMP_G+WC%VEG_TMP_L(I+LBURN))/2
...
! qe-
    DO J=0,NVEG_L-LBURN  !veg grid coordinate
      ...
      ENDDO
    ENDDO
    VEG_QRP_EMISS =  0.85_EB*VEG_QRP_EMISS*SIGMA
    VEG_QRM_EMISS =  0.85_EB*VEG_QRM_EMISS*SIGMA
!
    DO I=0,NVEG_L-LBURN
...
!   QRADP_INC = SIGMA*WC%VEG_TMP_L(NVEG_L)**4*EXP(-ETAVEG_H) + VEG_QRM_EMISS(NVEG_L-LBURN) !fds4
    ...
    DO I=0,NVEG_L-LBURN
      VEG_QRM_INC(I)   = 0.85_EB*QRADM_INC*SF%VEG_FINCM_RADFCT_L(I)
      VEG_QRP_INC(I)   = 0.85_EB*QRADP_INC*SF%VEG_FINCP_RADFCT_L(I)
...
!Newer version, includes char which becomes a heat sink since its not oxidized
        IF(TMP_VEG_NEW >= 400._EB .AND. MPA_VEG > MPA_VEG_MIN) THEN
          Q_UPTO_VOLIT = (CP_VEG*MPA_VEG + CP_CHAR*MPA_CHAR)*MAX((400._EB-TMP_VEG),0._EB)
...
REAL(EB) :: CFB,LX,SR_MAX,UMAX_LS,VMAX_LS
...
!---- Wind, combined wind & slope, and midflame windspeed factors
      CALL ROTH_WINDANDSLOPE_COEFF_HEADROS(NM,IIG,JJG,KKG,SF%VEG_LSET_BETA,SF%VEG_LSET_SURF_HEIGHT,SF%VEG_LSET_SIGMA, &
           SF%VEG_LSET_ROTH_ZEROWINDSLOPE_ROS)
    ENDIF
...
!************************************************************************************************
SUBROUTINE ROTH_WINDANDSLOPE_COEFF_HEADROS(NM,I,J,K,VEG_BETA,VEG_HT,VEG_SIGMA,ZEROWINDSLOPE_ROS)
!************************************************************************************************
!
...
REAL(EB), INTENT(IN) :: VEG_BETA,VEG_HT,VEG_SIGMA,ZEROWINDSLOPE_ROS
INTEGER :: KDUM,KWIND
REAL(EB) :: CONSFCTR,PHX,PHY,MAG_PHI
...
U_LS(I,J) = U(I,J,KWIND)
V_LS(I,J) = V(I,J,KWIND)
...
!PHI_W_X(I,J) = C_ROTH * ((3.281_EB * ABS(UMF_X(I,J)))**B_ROTH) * (VEG_BETA / BETA_OP_ROTH)**(-E_ROTH)
PHI_W_X(I,J) = CONSFCTR*(ABS(UMF_X(I,J)))**B_ROTH
PHI_W_X(I,J) = SIGN(PHI_W_X(I,J),UMF_X(I,J))

PHI_W_Y(I,J) = CONSFCTR*(ABS(UMF_Y(I,J)))**B_ROTH
!PHI_W_Y(I,J) = C_ROTH * ((3.281_EB * ABS(UMF_Y(I,J)))**B_ROTH) * (VEG_BETA / BETA_OP_ROTH)**(-E_ROTH)
PHI_W_Y(I,J) = SIGN(PHI_W_Y(I,J),UMF_Y(I,J))

PHI_W(I,J) =  SQRT(PHI_W_X(I,J)**2 + PHI_W_Y(I,J)**2) !used in LS vs FS paper
!UMF(I,J)   = UMF_TMP*SQRT(U_LS(I,J)**2 + V_LS(I,J)**2) * 60.00_EB
!PHI_W(I,J) = C_ROTH * ((3.281_EB * UMF(I,J))**B_ROTH) * (VEG_BETA / BETA_OP_ROTH)**(-E_ROTH)
...
!
! CFB = Crown Fraction Burned, needed for fireline intensity and heat input to atmosphere
! 

INTEGER,  INTENT(IN) :: I,J,K,NM
REAL(EB), INTENT(IN) :: CBD,EFFM,FSG,SFC,VERT_CANOPY_EXTENT
REAL(EB), INTENT(INOUT) :: CFB
CHARACTER(25), INTENT(IN) :: SURFACE_FIRE_HEAD_ROS_MODEL
INTEGER :: KDUM,KWIND
REAL(EB) :: CAC,CROSA,CROSP,CRLOAD,MPS_FCTR,EXPG,G,GMAX,PROB,PROB_PASSIVE,PROB_ACTIVE,PROB_CROWN,UMAG,UMF_TMP

!Find k array index corresponding to 10 m above canopy height to determine wind speed
KWIND = 0
KDUM  = K
DO WHILE (ZC(KDUM)-ZC(K) <= 10._EB)
  KWIND = KDUM
  KDUM  = KDUM + 1
ENDDO
IF (ZC(KBAR) < 10.0_EB) KWIND=1

UMAG = SQRT(U(I,J,KWIND)**2 + V(I,J,KWIND)**2)
MPS_FCTR = 1._EB/60._EB
...
CROSA = 11.02_EB*(UMAG**0.9_EB)*(CBD**0.19_EB)*EXP(-0.17_EB*EFFM)*MPS_FCTR 
CROSP = CROSA*EXP(-0.3333_EB*CROSA*CBD)*MPS_FCTR
...
!Compute ROS if crown fire exists
  IF(PROB >= PROB_CROWN) THEN
    ...
    ELSE
      ROS_HEAD(I,J) = CROSP
      CFB = CRLOAD*MAX(1.0_EB, (PROB - PROB_CROWN)/(1._EB - PROB_CROWN))
    ENDIF
  ENDIF
...
!Compute head fire rate of spread 
  IF (PROB < PROB_PASSIVE) THEN
   IF(SURFACE_FIRE_HEAD_ROS_MODEL .EQ. 'CRUZ') ROS_HEAD(I,J) = CROSP !else use surface ROS specified in input file
  ENDIF
  IF (PROB >= PROB_PASSIVE .AND. PROB < PROB_ACTIVE) ROS_HEAD(I,J) = CROSP 
  IF (PROB >= PROB_ACTIVE)                           ROS_HEAD(I,J) = CROSA

!Compute crown fraction burned (kg/m^2) for use in QCONF (heat input to atmosphere)
  CFB = 0.0_EB
  IF (PROB >= PROB_PASSIVE) THEN
    CRLOAD = CBD*VERT_CANOPY_EXTENT
    CFB    = CRLOAD*MAX(1.0_EB, (PROB - PROB_PASSIVE)/(PROB_ACTIVE - PROB_PASSIVE))
  ENDIF
...
REAL(EB) :: BURNOUT_FCTR,BURNOUT_TIME,CFB,FLI,HEAD_WIDTH_FCTR,IGNITION_WIDTH_Y,ROS_FLANK1,R_BURNOUT_FCTR,TIME_LS_LAST, &
...
!---Ellipse assumption with Rothermel head fire ROS (== FARSITE)
      IF (SF%VEG_LSET_SURFACE_FIRE_HEAD_ROS_MODEL=='ROTHERMEL') CALL ROTH_WINDANDSLOPE_COEFF_HEADROS(NM,IIG,JJG,KKG, &
...
!-- Cruz et al. crown fire head fire ROS model
    ...
    IF (PHI_LS(IIG,JJG) >= -SF%VEG_LSET_PHIDEPTH .AND. BURN_TIME_LS(IIG,JJG) <= BURNOUT_TIME) THEN 
     TOTAL_FUEL_LOAD = SF%VEG_LSET_SURF_LOAD + CFB
 ...
 DO IDUM=1,NX_LS
     DO JDUM=1,NY_LS
         IF (PHI_LS(IDUM,JDUM)>=-SF%VEG_LSET_PHIDEPTH .AND. TOA(IDUM,JDUM)<=-1.0_EB) THEN 
          ...
          FLI = SQRT(SR_Y_LS(IDUM,JDUM)**2 + SR_X_LS(IDUM,JDUM)**2)*0.001_EB* &
                  SF%VEG_LSET_HEAT_OF_COMBUSTION*(1.0_EB-SF%VEG_CHAR_FRACTION)*SF%VEG_LSET_SURF_LOAD
...
!Runtime output slice file containing level set function for smokeview
