...
IF_BOUNDARY_FILES: IF (N_BNDF>0 .AND. .NOT.EVACUATION_ONLY(NM)) THEN
   ...
   IF_NOT_APPEND: IF (.NOT.APPEND) THEN
      ...
      DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
         ...
         IF (ABS(IOR).NE.3. .OR. .NOT.BNDF_DEFAULT .AND. M%WALL(IW)%OBST_INDEX==0) M%INC(IOR,M%WALL(IW)%OBST_INDEX) = 0
      ENDDO
      M%NPATCH = 0
      DO N=0,M%N_OBST
         OB=>M%OBSTRUCTION(N)
      ...
      DO NN=1,PC%N_QUANTITIES
         SELECT CASE(PC%QUANTITIES_INDEX(NN))
            ...
            CASE(440)  ! PARTICLE WEIGHTING FACTOR
               QP(NPP,NN) = LP%PWT
            CASE(441)  ! VEG BULK DENSITY
               QP(NPP,NN) = LP%VEG_FUEL_MASS
         END SELECT
      ENDDO
   ENDDO LOAD_LOOP
...
REAL(FB) :: TMPUP,TMPLOW,ZINT,TMP_WGT,DELTA,RHO2,UVW,UODX,VODY,WODZ
...
SELECT CASE(INDX)
   ...
   CASE(12) ! BACK WALL TEMPERATURE
       SOLID_PHASE_OUTPUT = SUM(WC%VEG_FUELMASS_L(:))
...
!  R_SVxPR = 1.0_EB/PC%VEG_SV*LP%VEG_PACKING_RATIO
   DO N_TREE = 1, N_TREES_OUT
    ...
    TREE_OUTPUT_DATA_TOTAL(N_TREE,4) = TREE_OUTPUT_DATA_TOTAL(N_TREE,4) + TREE_OUTPUT_DATA(N_TREE,4,NM) !kg
    TREE_OUTPUT_DATA_TOTAL(N_TREE,5) = TREE_OUTPUT_DATA_TOTAL(N_TREE,5) + TREE_OUTPUT_DATA(N_TREE,5,NM) !kg
    TREE_OUTPUT_DATA_TOTAL(N_TREE,6) = TREE_OUTPUT_DATA_TOTAL(N_TREE,6) + TREE_OUTPUT_DATA(N_TREE,6,NM) !kg
    TREE_OUTPUT_DATA_TOTAL(N_TREE,7) = TREE_OUTPUT_DATA_TOTAL(N_TREE,7) + TREE_OUTPUT_DATA(N_TREE,7,NM) !kW
    TREE_OUTPUT_DATA_TOTAL(N_TREE,8) = TREE_OUTPUT_DATA_TOTAL(N_TREE,8) + TREE_OUTPUT_DATA(N_TREE,8,NM) !kW
    TREE_OUTPUT_DATA_TOTAL(N_TREE,9) = TREE_OUTPUT_DATA_TOTAL(N_TREE,9) + TREE_OUTPUT_DATA(N_TREE,9,NM) !# part
    TREE_OUTPUT_DATA_TOTAL(N_TREE,10) = TREE_OUTPUT_DATA_TOTAL(N_TREE,10) + TREE_OUTPUT_DATA(N_TREE,10,NM) 
    TREE_OUTPUT_DATA_TOTAL(N_TREE,11) = TREE_OUTPUT_DATA_TOTAL(N_TREE,11) + TREE_OUTPUT_DATA(N_TREE,11,NM) 
   ENDDO
