...
REAL(EB) :: DELKDELT,VC,DTDX,DTDY,DTDZ,TNOW,ZZ_GET(0:N_TRACKED_SPECIES), &
            ...
            TMP_G,TMP_WGT,DIV_DIFF_HEAT_FLUX,H_S,PBAR_D_RHO_H_S_DT,DT_SUBSTEP,UN,PROFILE_FACTOR
         ...
         CASE (SOLID_BOUNDARY)
            ...
            IF (WC%VENT_INDEX>0) THEN 
               ...
               IF (VT%N_EDDY>0) THEN ! Synthetic Eddy Method
                  ...
                  SELECT CASE(IOR)
                     CASE( 1)
                        WC%UWS = WC%UWS - TIME_RAMP_FACTOR*PRES_RAMP_FACTOR*VT%U_EDDY(JJ,KK)*SQRT(PROFILE_FACTOR)
                     CASE(-1)
                        WC%UWS = WC%UWS + TIME_RAMP_FACTOR*PRES_RAMP_FACTOR*VT%U_EDDY(JJ,KK)*SQRT(PROFILE_FACTOR)
                     CASE( 2)
                        WC%UWS = WC%UWS - TIME_RAMP_FACTOR*PRES_RAMP_FACTOR*VT%V_EDDY(II,KK)*SQRT(PROFILE_FACTOR)
                     CASE(-2)
                        WC%UWS = WC%UWS + TIME_RAMP_FACTOR*PRES_RAMP_FACTOR*VT%V_EDDY(II,KK)*SQRT(PROFILE_FACTOR)
                     CASE( 3)
                        WC%UWS = WC%UWS - TIME_RAMP_FACTOR*PRES_RAMP_FACTOR*VT%W_EDDY(II,JJ)*SQRT(PROFILE_FACTOR)
                     CASE(-3)
                        WC%UWS = WC%UWS + TIME_RAMP_FACTOR*PRES_RAMP_FACTOR*VT%W_EDDY(II,JJ)*SQRT(PROFILE_FACTOR)
                  END SELECT
               ENDIF
