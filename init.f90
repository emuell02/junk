   ...
   EDDY_IF: IF (VT%N_EDDY>0) THEN
      SELECT CASE(ABS(VT%IOR))
         CASE(1)
            ...
            !paceholder arrays for reference frame rotation
            ALLOCATE(VT%UT1(VT%J1+1:VT%J2,VT%K1+1:VT%K2),STAT=IZERO)
            CALL ChkMemErr('READ_VENT','UT1',IZERO)
            ALLOCATE(VT%UT2(VT%J1+1:VT%J2,VT%K1+1:VT%K2),STAT=IZERO)
            CALL ChkMemErr('READ_VENT','UT2',IZERO)
            ALLOCATE(VT%UT3(VT%J1+1:VT%J2,VT%K1+1:VT%K2),STAT=IZERO)
            CALL ChkMemErr('READ_VENT','UT3',IZERO)
         CASE(2)
            ...
            CALL ChkMemErr('READ_VENT','W_EDDY',IZERO)
            !paceholder arrays for reference frame rotation
            ALLOCATE(VT%UT1(VT%I1+1:VT%I2,VT%K1+1:VT%K2),STAT=IZERO)
            CALL ChkMemErr('READ_VENT','UT1',IZERO)
            ALLOCATE(VT%UT2(VT%I1+1:VT%I2,VT%K1+1:VT%K2),STAT=IZERO)
            CALL ChkMemErr('READ_VENT','UT2',IZERO)
            ALLOCATE(VT%UT3(VT%I1+1:VT%I2,VT%K1+1:VT%K2),STAT=IZERO)
            CALL ChkMemErr('READ_VENT','UT3',IZERO)
         CASE(3)
            ...
            CALL ChkMemErr('READ_VENT','W_EDDY',IZERO)
            !paceholder arrays for reference frame rotation
            ALLOCATE(VT%UT1(VT%I1+1:VT%I2,VT%J1+1:VT%J2),STAT=IZERO)
            CALL ChkMemErr('READ_VENT','UT1',IZERO)
            ALLOCATE(VT%UT2(VT%I1+1:VT%I2,VT%J1+1:VT%J2),STAT=IZERO)
            CALL ChkMemErr('READ_VENT','UT2',IZERO)
            ALLOCATE(VT%UT3(VT%I1+1:VT%I2,VT%J1+1:VT%J2),STAT=IZERO)
            CALL ChkMemErr('READ_VENT','UT3',IZERO)
      END SELECT
...
PROCESS_VENT: IF (I_VENT>0) THEN
   ...
   IF (SURFACE(SURF_INDEXX)%PROFILE==ATMOSPHERIC) THEN
      IF (M%ZC(K)<GROUND_LEVEL) THEN
         ...
         RETURN
      ENDIF
      IF (M%ZC(K).GE.12.5) THEN
          WC%UW0 =(0.87/.41)*LOG((M%ZC(K)-9.375)/0.4973)*WC%UW0
      ELSE
          WC%UW0 =3.9*EXP(3*(M%ZC(K)/12.5-1))*WC%UW0
      ENDIF
   ENDIF
