...
IF (FN_INPUT(1:1)==' ') THEN
   ...
   IF (MYID==0) THEN
      ...
      WRITE(LU_ERR,'(/A)')  "WFDS, October 27, 2015; based on subversion 9977 of FDS"
...
NAMELIST /MISC/ AL2O3,ALLOW_SURFACE_PARTICLES,ALLOW_UNDERSIDE_PARTICLES,ASSUMED_GAS_TEMPERATURE,BAROCLINIC,BNDF_DEFAULT,&
                ...
                C_FORCED_CYLINDER,C_FORCED_SPHERE,C_HORIZONTAL,C_VERTICAL,DEBUG_OPENMP,DNS,DT_MEAN_FORCING,EDDY_METHOD,ENTHALPY_TRANSPORT,&
                ...
                RESTART,RESTART_CHID,RUN_AVG_FAC,SC,SHARED_FILE_SYSTEM,SMOKE_ALBEDO,SOLID_PHASE_ONLY,&
                STRATIFICATION,TENSOR_DIFFUSIVITY,TERRAIN_CASE,TERRAIN_IMAGE,TEXTURE_ORIGIN, &
                THERMOPHORETIC_DEPOSITION,&
                THICKEN_OBSTRUCTIONS,TMPA,TURBULENT_DEPOSITION,TURBULENCE_MODEL,U0,UVW_FILE,V0,VEG_LEVEL_SET_COUPLED, &
                VEG_LEVEL_SET_UNCOUPLED,VISIBILITY_FACTOR,&
...
NAMELIST /DUMP/ COLUMN_DUMP_LIMIT,CTRL_COLUMN_LIMIT,DEBUG,DEVC_COLUMN_LIMIT,DT_BNDE,DT_BNDF,DT_CTRL,DT_DEVC,DT_DEVC_LINE,DT_FLUSH,&
                DT_GEOM,DT_HRR,DT_ISOF,DT_MASS,DT_PART,DT_PL3D,DT_PROF,DT_RESTART,DT_SL3D,DT_SLCF,DT_VEG,EB_PART_FILE,&
                FLUSH_FILE_BUFFERS,&
...
NAMELIST /PART/ AGE,BREAKUP,BREAKUP_CHILD_DIAMETER,BREAKUP_CHILD_GAMMA_D,BREAKUP_CHILD_SIGMA_D,CHECK_DISTRIBUTION,&
                ...
                VEG_BETA_CHAR,VEG_NU_O2_CHAR,VEG_NDT_SUBCYCLES
...
COUNT_VEG_LOOP: DO
   ...
   IF (FUEL_GEOM == 'CUSTOM')     N_CUSTOM_TREES = N_CUSTOM_TREES + 1
...
!
TREE_CASE = .TRUE.
print*,'read: N_TREES,N_CONE_TREES,N_FRUSTUM_TREES,N_CYLINDER_TREES,N_RECTANGLE_TREES,N_CUSTOM_TREES,N_RING_TREES', &
N_TREES,N_CONE_TREES,N_FRUSTUM_TREES,N_CYLINDER_TREES,N_RECTANGLE_TREES,N_CUSTOM_TREES,N_RING_TREES
...
!for array index used to find parameters only for custom veg volume
!arrays for custom volume specific parameters
NDIM = N_CUSTOM_TREES
IF (NDIM > 0) THEN
 ALLOCATE(TREE_CUSTOM_INDEX(N_TREES),STAT=IZERO)
 CALL ChkMemErr('READ','TREE_CUSTOM_INDEX',IZERO)
 ALLOCATE(XS_CUST_VEG(NDIM),STAT=IZERO)
 CALL ChkMemErr('READ','XS_RECT_VEG',IZERO)
 ALLOCATE(XF_CUST_VEG(NDIM),STAT=IZERO)
 CALL ChkMemErr('READ','XF_RECT_VEG',IZERO)
 ALLOCATE(YS_CUST_VEG(NDIM),STAT=IZERO)
 CALL ChkMemErr('READ','YS_RECT_VEG',IZERO)
 ALLOCATE(YF_CUST_VEG(NDIM),STAT=IZERO)
 CALL ChkMemErr('READ','YF_RECT_VEG',IZERO)
 ALLOCATE(ZS_CUST_VEG(NDIM),STAT=IZERO)
 CALL ChkMemErr('READ','ZS_RECT_VEG',IZERO)
 ALLOCATE(ZF_CUST_VEG(NDIM),STAT=IZERO)
 CALL ChkMemErr('READ','ZF_RECT_VEG',IZERO)
ENDIF
...
! contain vegetation
   ...
   IF (FUEL_GEOM == 'RECTANGLE'.OR.FUEL_GEOM=='CUSTOM') THEN
    X_TREE_MIN = XB(1)
    X_TREE_MAX = XB(2)
    Y_TREE_MIN = XB(3)
    Y_TREE_MAX = XB(4)
...
!
   ...
   ENDIF
!  
     IF (FUEL_GEOM=='CUSTOM') THEN
     N_CUSTOM_TREES  = N_CUSTOM_TREES + 1
     TREE_CUSTOM_INDEX(N) = N_CUSTOM_TREES
     XS_CUST_VEG(N_CUSTOM_TREES) = XB(1)
     XF_CUST_VEG(N_CUSTOM_TREES) = XB(2)
     YS_CUST_VEG(N_CUSTOM_TREES) = XB(3)
     YF_CUST_VEG(N_CUSTOM_TREES) = XB(4)
     ZS_CUST_VEG(N_CUSTOM_TREES) = XB(5)
     ZF_CUST_VEG(N_CUSTOM_TREES) = XB(6)
     IF (IGNITOR_ELEMENTS) THEN
      N_IGNITORS                      = N_IGNITORS + 1
      TREE_IGN_INDEX(N)               = N_IGNITORS
      TON_IGN_ELEMS(N_IGNITORS)       = TON_IGNITOR_ELEMENTS
...
REAL(EB) :: VEG_DRAG_COEFFICIENT,VEG_ASH_FRACTION,VEG_CHAR_FRACTION,VEG_DENSITY,VEG_HEIGHT, &
            VEGETATION_INITIAL_TEMP,VEG_LOAD,VEG_LSET_IGNITE_TIME,VEG_LSET_QCON,VEG_MOISTURE, &
            VEG_SV,FIRELINE_MLR_MAX,VEGETATION_GROUND_TEMP
REAL(EB) :: VEG_H_H2O,VEG_A_H2O,VEG_E_H2O,VEG_H_PYR,VEG_A_PYR,VEG_E_PYR,VEG_H_CHAR,VEG_A_CHAR,VEG_E_CHAR, &
            VEG_BETA_CHAR,VEG_NU_O2_CHAR,VEG_CHAR_ENTHALPY_FRACTION
REAL(EB) :: VEG_LSET_ROS_HEAD,VEG_LSET_ROS_FLANK,VEG_LSET_ROS_BACK, &
            VEG_LSET_WIND_EXP,VEG_LSET_BETA,VEG_LSET_SURF_HEIGHT,VEG_LSET_SIGMA,VEG_LSET_ROTH_ZEROWINDSLOPE_ROS, &
            VEG_LSET_PHIDEPTH, &
            ...
            VEG_LSET_CRUZ_PROB_ACTIVE,VEG_LSET_CRUZ_PROB_CROWN
LOGICAL :: VEG_LSET_HEADWIDTH_DEPENDENCE,VEGETATION,VEG_NO_BURN, &
           VEG_CHAR_OXIDATION,VEG_HCONV_CYLRE,VEG_HCONV_CYLLAM, &
           VEG_LEVEL_SET_SPREAD,VEG_LSET_IGNITE, &
...
NAMELIST /SURF/ ADIABATIC,BACKING,BOUNDARY_CONDITION_FILENAME,BURN_AWAY,CELL_SIZE_FACTOR,COLOR,CONVECTION_LENGTH_SCALE, &
                ...
                VEG_CHAR_OXIDATION,VEG_DEGRADATION,VEG_HCONV_CYLRE,VEG_HCONV_CYLLAM,VEG_DRAG_COEFFICIENT,&
                VEG_ASH_FRACTION,VEG_CHAR_FRACTION,VEG_DENSITY,VEGETATION_GROUND_TEMP,VEG_HEIGHT,&
                VEGETATION_INITIAL_TEMP,VEGETATION_LAYERS,VEG_LOAD,VEG_H_PYR, &
                VEG_H_H2O,VEG_A_H2O,VEG_E_H2O,VEG_H_PYR,VEG_A_PYR,VEG_E_PYR,VEG_H_CHAR,VEG_A_CHAR,VEG_E_CHAR, &
                VEG_BETA_CHAR,VEG_NU_O2_CHAR,VEG_CHAR_ENTHALPY_FRACTION, &
                VEG_LSET_IGNITE_TIME,VEG_LSET_IGNITE,&
                ...
                VEL,VEL_T,VOLUME_FLUX,WIDTH,XYZ,Z0,VEG_LSET_SIGMA,VEG_LSET_SURF_HEIGHT,&
                VEG_LSET_BETA,VEG_LSET_ELLIPSE,VEG_LSET_TAN2,VEG_LSET_ROTH_ZEROWINDSLOPE_ROS, &
...
READ_SURF_LOOP: DO N=0,N_SURF
   ...
   SF%VEG_LSET_SIGMA          = 0.01 * VEG_LSET_SIGMA !SAV for Farsite emulation in LSET, UNITS ARE 1/cm!
...
!                                             if used and burn time counter starts when phi >= -VEG_LSET_DEPTH
   ...
   SF%VEG_LSET_IGNITE              = VEG_LSET_IGNITE
...
!  SF%VEG_DEHYDRATION_RATE_MAX = SRF_VEG_DEHYDRATION_RATE_MAX
   ...
   SF%VEG_DRAG_INI = VEG_DRAG_COEFFICIENT*SF%VEG_PACKING*SF%VEG_SV
...
VEG_DEGRADATION              = 'LINEAR'
VEG_CHAR_OXIDATION           = .FALSE.
VEG_H_PYR                    = 418._EB !kJ/kg from Morvan
...
VEG_LSET_IGNITE              = .FALSE.
VEG_LSET_ROS_HEAD            = 0.0_EB
VEG_LSET_ROTH_ZEROWINDSLOPE_ROS = 1.0_EB !zero wind, zero slope ROS (m/s) from Rothermel 
...
VEG_LSET_CANOPY_HEIGHT       = 10._EB !m
